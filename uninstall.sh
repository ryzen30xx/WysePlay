...
# Remove custom compiled UxPlay binary and manpages if installed by WysePlay
if [[ -f /usr/local/bin/uxplay ]]; then
    rm -f /usr/local/bin/uxplay
    log_success "Đã xóa bản thực thi tùy biến: /usr/local/bin/uxplay"
fi
if [[ -f /usr/local/share/man/man1/uxplay.1 ]]; then
    rm -f /usr/local/share/man/man1/uxplay.1
    log_success "Đã xóa tài liệu manpage: /usr/local/share/man/man1/uxplay.1"
fi

# Remove SSH latency optimization config if created by WysePlay
if [[ -f /etc/ssh/sshd_config.d/99-wyseplay-fast-ssh.conf ]]; then
    rm -f /etc/ssh/sshd_config.d/99-wyseplay-fast-ssh.conf
    systemctl restart ssh 2>/dev/null || systemctl restart sshd 2>/dev/null || true
    log_success "Đã xóa cấu hình: /etc/ssh/sshd_config.d/99-wyseplay-fast-ssh.conf"
fi

# Remove temporary files and cached test artifacts
rm -f /tmp/airplay_* /tmp/uxplay.log /tmp/kiosk.log /tmp/sc_*.png /tmp/utm_*.png /tmp/fb0_current.jpg /tmp/test_*.png /tmp/current_screen.png /tmp/sample*.h264 2>/dev/null || true
log_success "Đã dọn dẹp các tệp tạm thời và nhật ký trong /tmp"
...
if [[ -n "$TARGET_USER" ]]; then
    ...
    if [[ -d "${TARGET_HOME}/.config/openbox" ]]; then
        ...
        if grep -qi "WifiKiosk\|airplay\|wyseplay\|gstreamer" "${TARGET_HOME}/.config/openbox/rc.xml" 2>/dev/null; then
