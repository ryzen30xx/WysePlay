...
def generate_wallpaper():
    ...
    if has_network:
        # --- FULLY CENTERED APPLE TV CONFERENCE ROOM DISPLAY LAYOUT ---
        center_x = W // 2

        # 1. Subtle Apple Ambient Glow behind center
        max_r = int(min(W, H) * 0.55)
        rad_img = Image.new('RGBA', (512, 512), (0, 0, 0, 0))
        rad_draw = ImageDraw.Draw(rad_img)
        for r in range(256, 0, -2):
            alpha = int(28 * (1.0 - r / 256.0))
        ...
        img.paste(rad_scaled, (center_x - max_r, int(H * 0.45) - max_r), rad_scaled)

        # 2. Dimensions calculation
        target_icon_w = int(120 * scale)
        ...
        pad_x, pad_y = int(26 * scale), int(12 * scale)
        ...
        # Draw Status
        draw.text((center_x - (b_stat[2] - b_stat[0]) // 2, current_y), stat_text, fill='#86868b', font=font_status)
        current_y += stat_h + gap_stat_net
        ...
        # --- NO NETWORK LAYOUT (Side-by-side with Wi-Fi panel) ---
        center_x = int(W * 0.72)

        # Ambient glow behind right half
        max_r = int(min(W, H) * 0.5)
        rad_img = Image.new('RGBA', (512, 512), (0, 0, 0, 0))
        rad_draw = ImageDraw.Draw(rad_img)
        for r in range(256, 0, -2):
            alpha = int(24 * (1.0 - r / 256.0))
            rad_draw.ellipse([256 - r, 256 - r, 256 + r, 256 + r], fill=(24, 42, 78, alpha))
        rad_scaled = rad_img.resize((max_r * 2, max_r * 2), Image.Resampling.BILINEAR)
        img.paste(rad_scaled, (center_x - max_r, int(H * 0.5) - max_r), rad_scaled)

        # Icon sizing
        target_icon_w = int(108 * scale)
        ratio = target_icon_w / icon_orig.width
        target_icon_h = int(icon_orig.height * ratio)
        icon_img = icon_orig.resize((target_icon_w, target_icon_h), Image.Resampling.LANCZOS)

        gap_icon_title = int(20 * scale)
        t_title = "AirPlay"
        b_title = draw.textbbox((0, 0), t_title, font=font_title)
        title_h = b_title[3] - b_title[1]

        gap_title_pill = int(24 * scale)
        lbl_txt = "Tên thiết bị: "
        val_txt = monitor_name
        b_lbl = draw.textbbox((0, 0), lbl_txt, font=font_label)
        b_val = draw.textbbox((0, 0), val_txt, font=font_name)
        ...
        # Warning status
        draw.text((center_x - (b_stat[2] - b_stat[0]) // 2, current_y), stat_text, fill='#ff9f0a', font=font_status)
        current_y += stat_h + gap_stat_inst
    ...
    out_file = '/opt/airplay/standby.png'
    final_img.save(out_file)
    print(f"Wallpaper saved: {out_file} (Monitor: {monitor_name}, Net: {net_type}, Res: {res})")
